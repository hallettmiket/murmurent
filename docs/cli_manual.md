...
# work in Claude Code on member/mike.hallett/qc-batch-3 ...
$ murmurent audit src/qc.py
$ murmurent push dcis_imaging --message "add QC for batch 3"
Pushed to member/mike.hallett/qc-batch-3 (direct; no review).

# later, when ready to merge:
$ murmurent push dcis_imaging --finalize
Opened PR #14: member/mike.hallett/qc-batch-3 → main
...
```
$ murmurent role assign lab_manager @lauren.smith
...
Acknowledged role-transition #42 (lab_manager → @lauren.smith)
```
...
$ murmurent project new dcis_imaging \
    --charter charter.md \
    --members @vanessa.dumeaux,@mike.hallett,@lauren.smith
