...
These are the same three classes of repository introduced in
[Overview: members, groups, projects](overview.md), described here with the
internal layout that matters at the group level. Scope is kept separate
across them.

- **The commons** (`hallettmiket/murmurent`): the shared agent definitions,
  rules, hooks, and install tooling. Centre-wide; cloned by everyone.
- **The lab-management repository** (`murmurent_lab_mgmt_<lab>`):
  group-scoped, one per group. Holds the role registry (`roles/`),
  inventory (`inventory/`), audit logs, the project registry, and
  group-wide protocols.
- **Project repositories**: project-scoped, one per active project. Each
  holds `MEMBERS` (the project's access list), `exp/`, `src/`, `findings/`,
  and the rest of the lab's standard project layout, private to its
  members. A project's primary repository may also carry a `CHARTER.md`, a
  short file recording the project's purpose and sensitivity tier; it is a
  legacy-but-supported marker that predates the current certificate-based
  project model (see [Creating a project](project_intra.md)).

## Agent classification: the freeze cascade

Agents ship as shared definitions in the commons, but a member may want to
customize an agent for their own work. The **freeze cascade** is the rule
that decides, for any given run, whether an agent uses the shared (frozen)
definition or a member's own (personal) version. "Freeze" marks an agent
whose definition should not drift from the shared version, for consistency
and reproducibility; "cascade" refers to the ordered levels below, where a
more specific level overrides a more general one.
...
## Customizing an agent, and keeping your changes

The commons agents are symlinked into `~/.claude/agents/` from your commons
clone, so a `git pull` on the commons updates them automatically. If you
...
Choreographies (recurring multi-actor workflow recipes, including the
finalisation choreography) are documented on their own page:
[Choreographies](choreography.md). This is an area of active development.
