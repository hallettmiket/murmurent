...
PI's admin token wigamig should use, then add the helper.
...
*lab's own* repo, not part of the wigamig commons. It belongs to the
PI, lives under the lab's GitHub org (`<labpi>/lab_mgmt`), and is
analogous to a department's filing cabinet — not a wigamig artifact.
...
`~/repos`. Every member is silently assumed to want their wigamig
...
**Problem.** The PI has a Mac .app bundle that one-clicks `wigamig
...
- **Linux** — `.desktop` file installed to
  `~/.local/share/applications/wigamig.desktop` (auto-installed by
  `wigamig install --hooks` on Linux). Includes an SVG icon.
- **Windows** — `.lnk` shortcut on the desktop, generated by a
  one-line PowerShell snippet. (`wigamig install --hooks` on Windows
...
## 8. Smoke-test polish: `wigamig install --hooks` warns the user not to share their `~/.wigamig/registrar` sentinel
...
- `wigamig install --hooks` should refuse to overwrite an existing
  `~/.wigamig/registrar` without `--force`.
- `wigamig centre-init` should default `--no-sentinel` when
