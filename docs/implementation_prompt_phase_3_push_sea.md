...
2. **SEA registry and operational verbs**
   ...
   - `murmurent sea request --to <m> --kind <k> --description <d>` — file an SEA, push to project
...
3. **Finalisation choreography**
   ...
   - `murmurent sea examine <id>` — scaffolds the deliberation doc with empty agent-contribution sections; sets `analysis_status: examined` after the squad fills sections (does NOT auto-invoke agents — students invoke them manually in their CC sessions)
   - `murmurent sea conclude <id> [--statement <path>]` — validates required sections present, prompts lead for the attempted statement, opens PR for squad approvals; sets `analysis_status: concluded` on merge
   - `murmurent finalize <scope> <id>` — umbrella that runs examine then prompts conclude; `<scope>` ∈ `sea`, `experiment`, `project`
   - `murmurent sea reopen <id>` — re-opens a concluded deliberation
