...
**As @mhallet (the requesting member):**
...
3. Switch to Slack → `#claude-test`. You should see:
   > 🎓 *biocore* training request — @mhallet wants @gary to train
   > them on `itc_basic_training` (**…**) (30 min, MSB 323).
...
6. Alert says "Signed off: mhallet on itc_basic_training". The roster
   card refreshes to show @mhallet with a green "current" pill.
...
- An event titled `MicroCal PEAQ-ITC … — @mhallet` lands on Gary's
  Google Calendar for the chosen time.
- Slack `#claude-test` gets:
  > 📅 Core *biocore*: @mhallet booked `itc_microcal_peaq` … — $80.00.
...
#    signed in as @mhallet (set $MURMURENT_USER if missing).
#    In CC, ask:
```
...
- "no MURMURENT_USER / USER set": the MCP runs without your shell env.
  Add `MURMURENT_USER=mhallet` to the env block of the MCP entry in
