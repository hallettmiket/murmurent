...
# 4. Dashboard boots.
wigamig dashboard
# Open http://localhost:8000 → log in as @the_pi (PI).
...
### Steps (as the booking member, e.g. `@the_pi`)

```bash
# 1. Confirm prereqs would block before training is set.
curl -s "http://localhost:8000/api/core/biocore/services/itc_microcal_peaq/can_book?member=@the_pi" | jq .
# Expect: {"ok": false, "training_slug": "itc_basic_training", "reason": "..."}

# 2. Seed the training record on @the_pi's member file.
$EDITOR ~/repos/lab_mgmt/members/the_pi.md
...
- An event titled "ITC — @the_pi" appears on Gary's Google Calendar
  for the chosen time.
- Slack #claude-test gets:
  `:calendar: Core *biocore*: @the_pi booked itc_microcal_peaq …`
