...
On the lab server (lab-server) close to the data. The user's local CC session connects via stdio over SSH (same pattern as the existing murmurent-oracle MCP).
