...
such as biodatsci) and surfaces permission, hygiene, and policy
...
## Why POSIX bits are not the whole story on OneFS

biodatsci (and any Schulich storage tier mounted from
`gnome-nfs.schulich.uwo.ca:/ifs/...`) exposes `/data` over **NFSv3**.
OneFS speaks NTFS-style ACLs natively, and the v3 mount can only
...
There's a parallel **NFSv4.1 mount at `/root/data4`** restricted to
...
`/home` on biodatsci is local ext4 (`/dev/md0`), so POSIX ACLs there
...
(Adapted from notes by Dr. Vanessa Dumeaux, who configured the OneFS
...
These are the canonical patterns Dumeaux set up for `/data/lab_vm/`.
...
A:fdg:Administrators@uwo.ca: rwaDdxtTnNcCoy          # admins: full, inherited
A::OWNER@                  : rwaDdxtTnNcCoy          # (duplicate owner-full)
A:fdi:OWNER@               : rwaDdxtTnNcCoy          # future child OWNER: full (inherit-only)
A:fdg:Users@uwo.ca         : rxtncy                  # UWO Users: READ everything, inherited
A:dg:Users@uwo.ca          : way                     # UWO Users on subdirs only: write/append/sync
```

Net effect: the lab group has full control; the broader `Users@uwo.ca`
...
```
A:fd:<named-user-1>@uwo.ca : rwaDdxtTnNcCoy   # explicit user: full, inherited
A:fd:OWNER@                : rwaDdxtTnNcCoy   # owner: full, inherited
A:fd:<named-user-2>@uwo.ca : rwaDdxtTnNcCoy   # second explicit user
A:fdg:Administrators@uwo.ca: rwaDdxtTnNcCoy   # admins: full, inherited
...
Net effect: the lab `ssmd-ud-vmlab` group is **deliberately locked out**;
...
```
A:fdg:ssmd-u-helpdesk@uwo.ca : rwaDdxtTnNcCoy   # helpdesk: full, inherited everywhere
A:fdg:Users@uwo.ca           : tcy              # all UWO users: read-attrs/ACL only, inherited
A::OWNER@                    : rwaDdxtTnNcCoy   # owner: full
A::GROUP@                    : rwaDxtTnNcy      # ssmd-ud-vmlab: full except delete-self & write-owner
A:fdg:Administrators@uwo.ca  : rwaDdxtTnNcCoy   # admins: full, inherited
...
similar inheritance — Dumeaux only configured the strict templates for
...
| <a id="CORE-REFINED-PATTERN-DRIFT-01"></a>`CORE-REFINED-PATTERN-DRIFT-01` | warn | A core's `refined/` root drifts from the canonical template (missing OWNER+GROUP full, missing Users@uwo.ca read, etc.). |
...
**local disk** of the host (NOT on the OneFS share — OneFS's NFSv4 ACLs
deny `root@<host>` write access since root isn't an AD principal there).
Owned by `root:ssmd-ud-vmlab`, mode 0750 (lab group reads; no one else).
Files:

| File | Contents |
|---|---|
| `manifest.json` | Script version, run timestamp, per-section status |
| `acls_raw.txt` | `nfs4_getfacl -R /root/data4/lab_vm/raw` |
| `acls_refined.txt` | `nfs4_getfacl -R /root/data4/lab_vm/refined` |
...
visibility — just blind to OneFS ACLs.
...
on notes from **Dr. Vanessa Dumeaux**, who configured the OneFS ACL
