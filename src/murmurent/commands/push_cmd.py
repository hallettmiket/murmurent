"""
Purpose: Implementations of ``murmurent push`` and ``murmurent pull``.
...
def cmd_push(
    ...
    *,
    ...
    """``murmurent push`` — personal branch push, finalize PR, or refined refresh."""
...
def cmd_pull(project_name: str) -> int:
    """``murmurent pull`` — fetch + fast-forward main."""
    repo = _resolve_repo(project_name)
...
def _push_finalize(repo: ProjectRepo, *, handle: str, message: str | None) -> int:
    ...
    body = (
        f"Opened by `murmurent push --finalize` for @{handle}.\n\n"
