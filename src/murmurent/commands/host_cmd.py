"""
Purpose: CLI handlers for ``murmurent host {list, add, remove, test}``.
...
def cmd_test(name: str) -> int:
    ...
    # Probe 2: murmurent --version
    click.echo("→ murmurent --version", nl=False)
    try:
    ...
    try:
        ...
        if res.ok:
            ...
            click.echo("  ⚠ (missing — murmurent will create on first project)")
