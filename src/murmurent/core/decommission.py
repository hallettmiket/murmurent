"""
Purpose: Soft-delete ("decommission") helper for murmurent entities.
...
Why a "decommission report" instead of `rm -rf`?
    murmurent must NEVER delete files in the user's installation. Every
...
def write_report(
    ...
    *,
    ...
    lines: list[str] = [
        ...
        f"Disconnected from murmurent by **{record.decommissioned_by}** at {ts}. ",
    ...
    if record.cleanup_items:
        lines.extend([
            ...
            "",
            "murmurent will NEVER touch these on your behalf. Review each item "
        ...
        lines.extend([
            "## Manual cleanup checklist",
            "",
            "_(none — murmurent did not detect any external resources to review.)_",
            "",
        ])
    lines.extend([
        "## Reverse this",
