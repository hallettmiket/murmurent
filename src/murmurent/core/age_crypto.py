...
key** (published in the murmurent_public directory) and emails the ciphertext.
...
def decrypt(ciphertext: str, key_path: Path | None = None) -> str:
    ...
    path = key_path or default_key_path()
    if not path.is_file():
        raise AgeError(f"no private key at {path}; run `murmurent centre-age-keygen` first.")
    try:
