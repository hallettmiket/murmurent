"""
Purpose: NFSv4 ACL parser + template diff for the murmurent Tier-2 audit.
...
Output: ``[Ace]`` per path + ``[Finding]`` from diffing against the
        ...
        (compiled from Dr. Dumeaux's NFSv4 reference).
...
# **file under raw/**. The Dumeaux reference shows files are born with
...
_REFINED_REQUIRED_ALLOW = (
    ...
    ("OWNER@", "", "rwaD"),
    ("GROUP@", "", "rwaD"),
    ("Administrators@uwo.ca", "fdg", "rwaDd"),
    ("Users@uwo.ca", "fdg", "r"),     # UWO read access (inherited)
)
...
def diff_raw(acls: list[FileAcl], *, host: str,
    ...
    raw_root_prefix = "lab_vm/raw"  # match suffix; the snapshot rewrites
                                     # /root/data4/lab_vm/raw -> .../lab_vm/raw
    for fa in acls:
        ...
        # but matches Dumeaux's templates which only put inheritance
        # flags on dir ACEs.
        ...
        if is_dir:
            for a in fa.aces:
                ...
                if (a.principal.startswith("Administrators@")
                    or a.principal.startswith("ssmd-u-helpdesk@")
                ...
                findings.append(_finding(
                    ...
                    expected="standard principal allowlist (OWNER@, GROUP@, helpdesk, Admins, Users@uwo.ca)",
        ...
        if is_root:
            ...
            if missing:
                findings.append(_finding(
                    ...
                    expected="OWNER+GROUP+Admins full; Users@uwo.ca rxtncy + way",
...
def _extract_project(path: str, kind: str) -> str | None:
    ...
    ``/data/lab_vm/wigamig/raw/<project>/`` on biodatsci where the
