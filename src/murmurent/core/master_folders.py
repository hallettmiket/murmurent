...
Input: A ``lab_base`` string in ``host:/abs/path`` form (e.g.
       ``lab-server.example.edu:/data/lab_vm/wigamig``).
...
def parse_lab_base(value: str | None) -> LabBase | None:
    ...
    # bracket alone (we don't expect those on lab-server but the strict
...
def _build_host(lab_base: LabBase) -> _hosts.Host:
    ...
    when lab-server-or-similar isn't yet in ``hosts.yaml``. The
...
def _build_batched_script(base_path: str, *, create: bool) -> str:
    ...
    Doing this in one shell snippet is the whole point: lab-server has a
...
def run(lab_base_value: str | None, *, create: bool) -> dict:
    ...
    # ONE batched SSH session — lab-server has a 3-strike auth lockout
