...
Purpose: First-time centre bootstrap ("mayor" flow). One person clones
         murmurent, runs ``murmurent centre-init``, ends up as the centre's
         first registrar.
...
Storage (single source of truth = ``<lab_info>/centre.md`` frontmatter):
  ...
  <lab_info>/centre.md
    ...
    server_host: 'lab-server.example.edu'   # the "murmurent server" hostname/IP
    server_account: 'murmurent'         # ssh login account on the server
    ...
    public_hub: 'github.com/hallettmiket/murmurent_public#western'
  ...
  "murmurent server"; the remaining fields drive the mayor's server-setup form.
...
class CentreProfile:
    ...
    server_host: str = ""                  # the "murmurent server" hostname/IP
    ...
    public_hub: str = ""                   # global murmurent_public + this centre's label
    raw_root: str = ""
    refined_root: str = ""
    created: str = ""
    path: Path | None = None
    ...
    @property
    def server(self) -> str:
        """The murmurent server hostname, preferring ``server_host`` and
...
def _render_centre(profile: CentreProfile) -> str:
    ...
    body = (
        "# Centre profile\n\n"
        f"This file declares the murmurent centre at **{profile.institution}**.\n"
...
def update_centre(
    ...
    *,
    ...
    if current is None:
        raise CentreInitError(
            "no centre initialised yet; run `murmurent centre-init` first."
        )
