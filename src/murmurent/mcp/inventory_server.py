"""
Purpose: Murmurent inventory MCP server. Exposes the markdown-backed
...
Input: stdio MCP protocol (the canonical CC integration), or direct calls via
       :mod:`murmurent.mcp.inventory_server.tools` for the test harness.
Output: JSON-serialisable structures the MCP client renders for the model.

Run as a server::

    python -m murmurent.mcp.inventory_server
...
def _build_server():  # pragma: no cover - exercised only when mcp is installed
    ...
    server = FastMCP(
        name="murmurent-inventory",
        instructions=(
            "Murmurent group inventory. Use `inventory_list` for browsing "
