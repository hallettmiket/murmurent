"""
Purpose: ``wigamig security scan`` — run the Tier-1 unprivileged security
...
Mirrors :mod:`wigamig.commands.reconcile_cmd` — a thin click wrapper
...
def add_to_cli(cli_group: click.Group) -> None:
    """Attach the ``wigamig security`` subcommand group."""
    ...
    @click.option("--host", required=True, help="Registered host name (see `wigamig host list`).")
    ...
    @click.option("--lab-group", default=None, help="Lab Unix group (e.g. ssmd-u-hallettlab).")
