"""
Purpose: ``wigamig project`` — centre_cable_guy's CLI front door for
...
Examples:
    ...
    wigamig project declare --name dcis_imaging --primary-lab hallett \\
      --member @allie --member @didi --member @cara \\
      --machine biodatsci \\
      --github-org hallettmiket --github-repo dcis_imaging

    # List declared projects.
    wigamig project list

    # Show one project.
    wigamig project show dcis_imaging

    # Reconcile (dry-run by default).
    wigamig project reconcile dcis_imaging
    wigamig project reconcile dcis_imaging --apply

    # Append an audit-log entry by hand.
    wigamig project log dcis_imaging --action "manual fs ACL fix" \\
