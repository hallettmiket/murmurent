"""
Purpose: Implementations of ``wigamig experiment ...`` subcommands.
...
def cmd_new(
    ...
    *,
    ...
    """``wigamig experiment new`` — scaffold ``exp/<n>_<slug>/`` and lab-VM dirs."""
...
def cmd_list(project_name: str | None) -> int:
    """``wigamig experiment list`` — print all experiments and their statuses."""
...
def cmd_status(project_name: str, slug: str, set_value: str) -> int:
    """``wigamig experiment status`` — flip the notebook ``status`` field."""
...
def cmd_ingest(
    ...
    *,
    ...
    """``wigamig experiment ingest`` — classify, prompt, copy, chmod, hash, update notebook."""
    repo = _resolve_repo(project_name)
    exp_dir = _resolve_experiment(repo, slug)
