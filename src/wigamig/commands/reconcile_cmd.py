"""
Purpose: ``wigamig reconcile`` — compare wigamig's recorded state to
...
def cmd_reconcile(*, apply: bool, slack_body: bool) -> int:
    ...
    routine that wraps this CLI) — wigamig doesn't ship a non-MCP
...
def _render_table(report: _rec.ReconcileReport) -> None:
    ...
    console = Console()
    console.print(f"\n[bold]Wigamig reconcile[/] — {report.generated_at}")
...
def _build_slack_body(report: _rec.ReconcileReport, *, applied: bool) -> str:
    ...
    per rules/slack.md."""
    head = f"*wigamig reconcile* — {report.generated_at}\n{report.summary_line()}"
    ...
    if applied:
        ...
        lines.append(
            "\n_Dry-run only. Run `wigamig reconcile --apply` "
...
def add_to_cli(cli_group: click.Group) -> None:
    """Attach the ``wigamig reconcile`` subcommand to the CLI group."""

    @cli_group.command("reconcile", help="Detect drift between wigamig's state and on-disk reality.")
