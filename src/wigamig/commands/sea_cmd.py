"""
Purpose: Implementations of ``wigamig sea ...`` subcommands.
...
def cmd_request(
    *,
    ...
    """``wigamig sea request`` — file a new SEA inside a project."""
...
def cmd_list(
    *,
    ...
    """``wigamig sea list`` — print SEAs filtered by direction."""
...
def cmd_complete(sea_id: int, *, delivery: str, project_name: str | None = None) -> int:
    """``wigamig sea complete <id> --delivery <path>``."""
    repo, s = _load(sea_id, project_name)
    try:
...
def cmd_decline(sea_id: int, *, reason: str, project_name: str | None = None) -> int:
    """``wigamig sea decline <id> --reason <r>``."""
    repo, s = _load(sea_id, project_name)
    try:
...
def cmd_examine(sea_id: int, *, project_name: str | None = None) -> int:
    """``wigamig sea examine <id>`` — scaffold the deliberation document."""
    repo, s = _load(sea_id, project_name)
    ...
    click.echo(
        ...
        "the relevant section. Run `wigamig sea conclude` when ready."
    )
    return 0
...
def cmd_conclude(
    ...
    *,
    ...
    """``wigamig sea conclude <id>`` — validate sections, mark concluded."""
    repo, s = _load(sea_id, project_name)
    delib_path = deliberation.deliberation_path(repo, "sea", str(s.id))
    if not delib_path.exists():
        raise click.ClickException(
            f"No deliberation at {delib_path}; run `wigamig sea examine {s.id}` first."
        )
    ...
    click.echo("Run `wigamig push <project> --finalize` to open a PR for squad approvals.")
    return 0


def cmd_reopen(sea_id: int, *, project_name: str | None = None) -> int:
    """``wigamig sea reopen <id>`` — re-open a concluded deliberation."""
    repo, s = _load(sea_id, project_name)
    try:
        sea.reopen(s)
    except sea.SeaTransitionError as exc:
...
def cmd_finalize(scope: str, target_id: str, *, project_name: str | None = None) -> int:
    """``wigamig finalize <scope> <id>`` — examine then conclude."""
    if scope == "sea":
