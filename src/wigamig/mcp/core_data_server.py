"""
Purpose: ``wigamig-core-data`` MCP server. Exposes a core's per-job
...
Run as a server::
    ...
    python -m wigamig.mcp.core_data_server
...
Design (Phase 5d of the cores rollout, plan §8):
  ...
  - The MCP runs on the lab server, close to ``$WIGAMIG_LAB_VM_ROOT``.
    ...
    pattern as the existing ``wigamig-oracle`` server).
  - Identity comes from ``$WIGAMIG_USER`` set by the wigamig shell
