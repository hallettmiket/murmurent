...
member clones the wigamig repo — the first real ``wigamig`` command they run
mints THIS machine's ed25519 signing keypair, whose fingerprint is their unique
wigamig ID (the thing a PI later binds an identity card to).
...
The explicit ``wigamig identity-init`` command calls ``idkeys.generate_keypair``
...
def local_identity(*, allow_unknown: bool = True) -> dict:
    ...
    exists (``wigamig identity-init`` or the first auto-run)."""
