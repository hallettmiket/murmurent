...
raw/refined/notebook mkdir). On biodatsci, where a 3-strike auth
lockout costs 30 minutes, that's wasteful. This module produces a
single bash snippet that:

  1. Probes ``wigamig --version`` on the host (required).
...
the user already established for biodatsci, an install is typically
...
def build_script(t: InstallTargets) -> str:
    ...
    # 1) wigamig presence (REQUIRED). bash -lc gives us login PATH so
    # ~/.local/bin is on it without us hardcoding it.
    lines.append(
        'WIGV=$(wigamig --version 2>&1 | head -1); '
        'if [ -n "$WIGV" ] && echo "$WIGV" | grep -q -i wigamig; '
        'then echo "wigamig:ok:$WIGV"; '
        'else echo "wigamig:fail:$WIGV  -- run scripts/install_remote.sh on your laptop"; fi'
    )
    # 2) per-project dirs.
    for label, path in (
        ("raw", raw_proj),
    ...
    if t.repo_url:
        ...
        # clone landed and the wigamig commons is present on the host
        # (cloned by ``scripts/install_remote.sh`` to ~/repos/wigamig).
        # Each picked agent becomes a symlink into the wigamig commons
        ...
        lines.append(
            ...
            # — but only delete symlinks that point into the wigamig
            ...
            f'    else '
            f'      echo "cc_agent:warn:$a (no $a.md in wigamig commons)"; '
            f'    fi; '
            f'  done; '
            ...
            f'# {project}\n'
            f'\n'
            f'Auto-generated by wigamig at install time. Replace this stub with\n'
            ...
            f'  fi; '
            f'else '
            f'  echo "cc_init:warn:skipped (clone missing or wigamig commons not at $WIG)"; '
            f'fi'
        )
...
def parse_output(stdout: str) -> list[Probe]:
    ...
    Steps that are required for the install to be considered successful:
      - ``wigamig``: install can't proceed without the binary on the host
