...
class ScanOptions:
    ...
    lab_group: str | None = None           # e.g. labgroup
...
def _read_scanner_script() -> str:
    """Locate ``scripts/wigamig_sec_scan.sh`` in the wigamig repo."""
...
def scan(host_obj: _hosts.Host, opts: ScanOptions | None = None,
    ...
    A scan over ``/data/lab_vm`` on lab-server can legitimately take a
