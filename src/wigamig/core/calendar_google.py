...
Input: Per-core OAuth credentials at
       ...
       ``wigamig core-calendar-auth`` via the InstalledAppFlow).
...
def run_oauth_flow(core: str) -> Path:
    ...
    Requires ``google-auth-oauthlib`` (``pip install wigamig[gcal]``).
...
def _load_credentials(core: str):
    ...
    try:
        ...
        raise CalendarError(
            "google-auth not installed; run: pip install 'wigamig[gcal]'"
        ) from exc
    cp = creds_path(core)
    if not cp.is_file():
        raise CalendarError(
            f"calendar not connected for core {core!r}; "
            f"the leader must run: wigamig core-calendar-auth --core {core}"
        )
