...
- **Hard rule (carried from the global wigamig charter)**: every
...
# /data/lab_vm/{raw,refined} guardrail (defence in depth — the wigamig
...
_SYSTEM_PROMPT = """\
You are the wigamig `security_guard` agent operating in "agent review" mode for
...
## Hard rule — wigamig data immutability
...
- `permissions.allow` entries that match destructive patterns under
  ...
  by the wigamig hooks but should also not appear in allow lists.
