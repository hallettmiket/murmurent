"""
Purpose: One-place SSH chokepoint for every remote operation wigamig makes.
Author: Mike Hallett (with Claude Code)
Date: 2026-05-13
Input: A :class:`~wigamig.core.hosts.Host` of kind ``ssh``.
...
class Remote:
    ...
    def wigamig_version(self) -> str:
        """Return the host's ``wigamig --version`` output (raises if missing)."""
        # `wigamig` may live under ~/.local/bin which the remote login
        # shell adds via .profile — the bash -lc wrapper handles that.
        res = self.run("wigamig --version", timeout=20)
