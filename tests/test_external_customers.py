...
def test_resolve_local_lab(world):
    ...
    assert "@mhallet" in res.pi_or_contact
...
def test_http_create_list_update_archive(world):
    client = TestClient(create_app())
    res = client.post(
        "/api/registrar/external_customers?user=mhallet",
        json={"id": "acme", "name": "ACME", "po_number": "PO-1"},
    )
    assert res.status_code == 200, res.text
    res = client.get("/api/registrar/external_customers?user=mhallet")
    assert any(c["id"] == "acme" for c in res.json()["customers"])
    res = client.patch(
        "/api/registrar/external_customers/acme?user=mhallet",
        json={"po_number": "PO-2"},
    )
    assert res.status_code == 200
    assert EC.get_customer("acme").po_number == "PO-2"
    res = client.post(
        "/api/registrar/external_customers/acme/archive?user=mhallet",
    )
    assert res.status_code == 200
...
def test_http_patch_unknown_id_404(world):
    client = TestClient(create_app())
    res = client.patch(
        "/api/registrar/external_customers/ghost?user=mhallet",
        json={"po_number": "PO-9"},
    )
