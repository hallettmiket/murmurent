...
def world(monkeypatch, tmp_path):
    ...
    project_cmd.cmd_new(
        "p_cat", charter_path=None, members_csv="@mhallet,@allie",
...
def test_snapshot_inbound_only_visible_to_pi(world):
    ...
    pi = snapshot.build_response("mhallet", today=_dt.date(2026, 5, 8))
...
def test_endpoint_catalog_upsert_pi_only(world):
    ...
    # PI succeeds
    res = client.post("/api/sea_catalog?user=mhallet", json=body)
...
def test_endpoint_catalog_disable_then_enable(world):
    ...
    client.post("/api/sea_catalog?user=mhallet", json=body)
    res = client.post("/api/sea_catalog/ep2/disable?user=mhallet")
    assert res.status_code == 200
    assert res.json()["accepting"] is False
    res = client.post("/api/sea_catalog/ep2/enable?user=mhallet")
    assert res.status_code == 200
...
def test_endpoint_catalog_delete_404_for_unknown(world):
    client = _client()
    res = client.post("/api/sea_catalog/nonexistent/delete?user=mhallet")
...
def test_endpoint_inbound_simulate_then_accept(world):
    ...
    # PI accepts
    res3 = client.post(
        f"/api/inbound-sea/{rid}/accept?user=mhallet", json={"routed_to": "@allie"}
    )
...
def test_endpoint_inbound_decline_requires_reason(world):
    ...
    rid = sim["request"]["id"]
    res = client.post(f"/api/inbound-sea/{rid}/decline?user=mhallet", json={})
