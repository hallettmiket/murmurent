...
def world(monkeypatch, tmp_path):
    ...
    project_cmd.cmd_new(
        "p_slk", charter_path=None, members_csv="@mhallet,@allie",
        description="x", sensitivity="standard", lead="@mhallet",
        skip_github=True,
    )
    return tmp_path
...
def test_token_from_env_wins(monkeypatch):
    monkeypatch.setenv("MURMURENT_SLACK_TOKEN", "xoxb-abc")
...
def test_oracle_drafts_visible_to_pi_only(world):
    ...
    pi_resp = snapshot.build_response("mhallet", today=_dt.date(2026, 5, 8))
...
def test_approve_draft_flips_status(world):
    ...
    distill.approve_draft(path, approver="mhallet")
    text = path.read_text()
    assert "status: published" in text
    assert "approved_by: '@mhallet'" in text or "approved_by: \"@mhallet\"" in text or "approved_by: '@mhallet'" in text
...
def test_endpoint_approve_pi_only(world):
    ...
    res_pi = client.post("/api/oracle/d1/approve?user=mhallet", json={})
    assert res_pi.status_code == 200


def test_endpoint_approve_404_for_unknown(world):
    client = _client()
    res = client.post("/api/oracle/nope/approve?user=mhallet", json={})
...
def test_endpoint_approve_409_when_not_draft(world):
    ...
    client = _client()
    res = client.post("/api/oracle/published_thing/approve?user=mhallet", json={})
...
def test_endpoint_decline_requires_reason(world):
    _seed_oracle_file(world, slug="d1", status="draft")
    client = _client()
    res = client.post("/api/oracle/d1/decline?user=mhallet", json={})
