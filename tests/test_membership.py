...
def test_add_refuses_existing_handle(world):
    with pytest.raises(M.MemberAlreadyExists):
        M.add(handle="the_pi", full_name="x", role="pi")
...
def test_set_status_pi_protected(world):
    with pytest.raises(M.CannotDeactivatePI):
        M.set_status("the_pi", M.INACTIVE)
...
def test_iter_members_filters(world):
    ...
    assert all_handles == {"the_pi", "a", "b"}
    assert active == {"the_pi", "a"}
...
def test_endpoint_add_member_pi_only(world):
    ...
    res_pi = client.post("/api/members?user=the_pi", json=body)
...
def test_endpoint_deactivate_then_activate(world):
    M.add(handle="bob", full_name="B", role="postdoc")
    client = _client()
    res = client.post("/api/members/bob/deactivate?user=the_pi")
    assert res.status_code == 200
    assert res.json()["status"] == "inactive"
    res2 = client.post("/api/members/bob/activate?user=the_pi")
...
def test_endpoint_cannot_deactivate_pi(world):
    client = _client()
    res = client.post("/api/members/the_pi/deactivate?user=the_pi")
    assert res.status_code == 409


def test_endpoint_404_for_missing_member(world):
    client = _client()
    res = client.post("/api/members/ghost/deactivate?user=the_pi")
...
def test_inactive_member_cannot_run_sea_action(world, monkeypatch):
    ...
    project_cmd.cmd_new(
        "p_act", charter_path=None, members_csv="@the_pi,@allie,@bob",
