...
def test_http_create_as_registrar(mock_post, world):
    ...
    res = client.post(
        "/api/core/biocore/services?user=mhallet",
...
def test_http_create_unknown_core(world):
    client = TestClient(create_app())
    res = client.post(
        "/api/core/ghost/services?user=mhallet",
        json={"slug": "x", "name": "X"},
    )
...
def test_http_update_patches_field(mock_post, world):
    ...
    client = TestClient(create_app())
    res = client.patch(
        "/api/core/biocore/services/abc?user=mhallet",
...
def test_http_archive_endpoint(mock_post, world):
    ...
    client = TestClient(create_app())
    res = client.post(
        "/api/core/biocore/services/abc/archive?user=mhallet",
    )
