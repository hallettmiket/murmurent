...
def test_detect_orphan_installation_remote_ssh_probe(world, monkeypatch):
    ...
    _write_manifest(world["installations"], "still_here", ssh_remote="biodatsci")
    _write_manifest(world["installations"], "deleted_there", ssh_remote="biodatsci")
    ...
    def fake_run(self, command, *, check=True, timeout=30):
        ...
        return _remote.RemoteResult(host="biodatsci", command=command,
...
def test_ssh_probe_failure_is_conservative(world, monkeypatch):
    ...
    _hosts.add(_hosts.Host(name="biodatsci", kind="ssh", ssh_host="biodatsci",
                           project_root="/home/u/repos"))
    _write_manifest(world["installations"], "maybe_alive", ssh_remote="biodatsci")
...
def test_detect_unadopted_clones_from_cached_inventory(world):
    ...
    assert "local" in by_host and "biodatsci" in by_host
    assert by_host["local"].severity == "info"
    assert "1 git clones" in by_host["local"].detail
    assert "1 git clones" in by_host["biodatsci"].detail
