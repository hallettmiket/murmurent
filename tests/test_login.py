...
def test_registry_round_trip_with_registrars(isolated):
    original = Registry(
        labs=[LabEntry(name="hallett", pi="@mhallet", lab_mgmt_path="/tmp/x")],
        registrars=["mhallet", "alice"],
    )
    registrar.write_registry(original)
    reread = registrar.read_registry()
    assert reread.registrars == ["mhallet", "alice"]
...
def test_registrars_normalises_and_dedups(isolated):
    ...
    registrar.registry_path().write_text(
        "version: 1\nregistrars: ['@MHallet', 'alice', '@mhallet']\nlabs: {}\n",
        encoding="utf-8",
    )
    assert registrar.registrars() == ["mhallet", "alice"]


def test_is_registrar_prefers_registry_list_over_sentinel(isolated):
    # Sentinel says mhallet; registry list says alice. Registry wins.
...
def test_is_registrar_multiple_centres(isolated):
    ...
    registrar.write_registry(Registry(registrars=["mhallet", "alice"]))
    assert registrar.is_registrar("mhallet") is True
    assert registrar.is_registrar("@Alice") is True
...
def test_role_audit_record_appends_jsonl(isolated):
    role_audit.record(handle="mhallet", role="pi", source="127.0.0.1", allowed=True)
...
def test_role_audit_read_all_round_trip(isolated):
    ...
    role_audit.record(handle="mhallet", role="pi", source="127.0.0.1",
                      allowed=True, now=now)
    events = role_audit.read_all()
    assert len(events) == 1
    assert events[0].handle == "mhallet"
...
def test_role_audit_recent_for_filters_handle(isolated):
    ...
    role_audit.record(handle="mhallet", role="registrar", source="127.0.0.1", allowed=True)
    rows = role_audit.recent_for("mhallet")
...
def test_login_resolve_member_only(isolated, tmp_path, monkeypatch):
    lab_dir = _seed_lab_mgmt(tmp_path, lab_id="hallett", pi="mhallet",
...
def test_login_resolve_registrar_from_registry(isolated, tmp_path, monkeypatch):
    ...
    assert "mhallet" in body["registrar_centres"]
...
def test_login_select_registrar_allowed(isolated, tmp_path, monkeypatch):
    ...
    reg = registrar.read_registry()
    registrar.write_registry(Registry(
        labs=reg.labs, registrars=["mhallet"],
    ))
    client = TestClient(create_app())
    res = client.post("/api/login/select", json={
        "handle": "mhallet", "role": "registrar", "remember_user": False,
    })
    assert res.status_code == 200, res.text
    body = res.json()
    assert body["next"] == "/registrar?user=mhallet"
