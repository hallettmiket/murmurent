...
@pytest.fixture
def biodatsci(world):
    """Register a `biodatsci` host so cmd_new_remote can resolve it."""
...
def test_render_registry_entry_local_unchanged(world):
    summary = _projects.ProjectSummary(
        ...
        sensitivity="standard", lead="@mhallet",
        members=("@mhallet",), choreography=None,
    )
...
def test_render_registry_entry_with_host(world):
    ...
    text = _projects.render_registry_entry(
        ...
        host_name="biodatsci", remote_path="/home/mhallet/repos/x",
    )
    assert "host: biodatsci" in text
    assert "remote_path: /home/mhallet/repos/x" in text
...
def test_render_registry_entry_remote_requires_path(world):
    ...
    with pytest.raises(ValueError):
        _projects.render_registry_entry(
            summary, today="2026-05-13", host_name="biodatsci", remote_path="",
        )
...
def test_is_remote_pointer_true_when_marker_present(tmp_path):
    ...
    (d / "CHARTER.md").write_text(
        "---\nproject: proj\nhost: biodatsci\nremote_path: /home/mhallet/repos/proj\n---\n",
        encoding="utf-8",
    )
    assert _projects.is_remote_pointer(d) is True
    host, path = _projects.read_remote_pointer(d)
    assert host == "biodatsci"
    assert path == "/home/mhallet/repos/proj"
...
def test_cmd_new_remote_writes_pointer_and_registry(world, biodatsci, fake_ssh):
    remote_path = project_cmd.cmd_new_remote(
        "myproj",
        host_name="biodatsci",
        members_csv="mhallet,alice",
        sensitivity="standard",
        description="a great project",
    )
    assert remote_path == "/home/mhallet/repos/myproj"
    ...
    assert meta["host"] == "biodatsci"
    assert meta["remote_path"] == "/home/mhallet/repos/myproj"
    # lab-mgmt registry entry has host: + remote_path:
    registry = world["lab_mgmt"] / "cert_projects" / "myproj.md"
    assert registry.is_file()
    text = registry.read_text(encoding="utf-8")
    assert "host: biodatsci" in text
    assert "remote_path: /home/mhallet/repos/myproj" in text


def test_cmd_new_remote_sends_expected_command_over_ssh(world, biodatsci, fake_ssh):
...
def test_cmd_new_remote_unknown_host_refused(world):
    with pytest.raises(click.ClickException) as exc_info:
        project_cmd.cmd_new_remote(
            "x", host_name="ghost", members_csv="mhallet", sensitivity="standard",
        )
...
def test_cmd_new_remote_local_host_refused(world):
    with pytest.raises(click.ClickException) as exc_info:
        project_cmd.cmd_new_remote(
            "x", host_name="local", members_csv="mhallet", sensitivity="standard",
        )
    assert "local" in str(exc_info.value)


def test_cmd_new_remote_needs_sensitivity(world, biodatsci):
    with pytest.raises(click.ClickException) as exc_info:
        project_cmd.cmd_new_remote(
            "x", host_name="biodatsci", members_csv="mhallet", sensitivity=None,
        )
    assert "sensitivity" in str(exc_info.value)


def test_cmd_new_remote_ssh_unreachable_raises(world, biodatsci, monkeypatch):
    ...
    with pytest.raises(click.ClickException) as exc_info:
        project_cmd.cmd_new_remote(
            "x", host_name="biodatsci", members_csv="mhallet", sensitivity="standard",
        )
...
def test_cmd_new_remote_refuses_to_overwrite_non_pointer(world, biodatsci, fake_ssh):
