...
def test_http_add_member(mock_post, world):
    ...
    res = client.post(
        "/api/registrar/core/biocore/members?user=the_pi",
...
def test_http_remove_member(mock_post, world):
    ...
    client = TestClient(create_app())
    res = client.post(
        "/api/registrar/core/biocore/members/alice/remove?user=the_pi",
    )
    assert res.status_code == 200, res.text
...
def test_http_rotate_leader(mock_post, world):
    ...
    client = TestClient(create_app())
    res = client.post(
        "/api/registrar/core/biocore/leader?user=the_pi",
...
def test_http_remove_leader_refuses(world):
    client = TestClient(create_app())
    res = client.post(
        "/api/registrar/core/biocore/members/gary/remove?user=the_pi",
    )
...
def test_http_unknown_core_returns_404(world):
    client = TestClient(create_app())
    res = client.post(
        "/api/registrar/core/ghost/members?user=the_pi",
        json={"handle": "alice"},
    )
