...
def pi_world(monkeypatch, tmp_path):
    ...
    (tmp_path / "lab-mgmt" / "lab.md").write_text(
        "---\nlab: lab_mh\nname: lab_mh\npi: '@mhallet'\n---\n", encoding="utf-8")
    K.generate_keypair()
    ISS.self_issue_pi_card("@mhallet", "lab_mh")   # PI card + registers lab + PI member
    return tmp_path
...
def test_issue_card_endpoint_certifies_and_audit_sees_valid(pi_world):
    ...
    res = client.post("/api/members/issue-card?user=mhallet",
                      json={"enrollment": enrollment, "group": "lab_mh", "dm": False})
    ...
    audit = client.get("/api/members/audit?user=mhallet").json()
