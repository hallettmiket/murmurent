"""Once a centre is live with a private mayor channel, murmurent system
...
def test_route_redirects_default_to_mayor_channel(monkeypatch):
    monkeypatch.setattr("murmurent.core.centre_init.read_centre",
...
def test_route_falls_back_to_dev_when_no_mayor_channel(monkeypatch):
    # no centre at all
    monkeypatch.setattr("murmurent.core.centre_init.read_centre", lambda *a, **k: None)
...
def test_route_never_raises(monkeypatch):
    ...
    monkeypatch.setattr("murmurent.core.centre_init.read_centre", boom)
