...
Uses fixture snapshot text built from Dr. Core Lead's NFSv4 reference
(see docs/security-dashboard.md). The dump format is `nfs4_getfacl -R`:

    # file: /srv/acl-view/lab_vm/raw
    A:fdg:Users@example.edu:tcy
    D:fdi:OWNER@:Dd
    ...
    # file: /srv/acl-view/lab_vm/raw/dcis/sample.bam
    A:fi:OWNER@:rxtTcy
    ...
...
def test_parse_single_ace_line():
    ...
    assert len(out) == 1
    assert out[0].path == "/srv/acl-view/lab_vm/raw"
    assert len(out[0].aces) == 1
    ace = out[0].aces[0]
    assert ace.type == "A"
    assert ace.flags == "fdg"
    assert ace.principal == "Users@example.edu"
...
def test_parse_multiple_files():
    text = (
        ...
        "A::OWNER@:rwaDdxtTnNcCoy\n"
        "\n"
        "# file: /srv/acl-view/lab_vm/raw/dcis/x.bam\n"
        "A:fi:OWNER@:rxtTcy\n"
        "A:fi:GROUP@:rxtTcy\n"
    )
    out = A.parse_nfs4_getfacl(text)
...
# Canonical raw/ root ACL (Core Lead's template).
RAW_ROOT_OK = (
    "# file: /srv/acl-view/lab_vm/raw\n"
    "D::OWNER@:Dd\n"
    "D::GROUP@:Dd\n"
...
RAW_FILE_WRITABLE = (
    "# file: /srv/acl-view/lab_vm/raw/dcis/leaked.bam\n"
    "A::OWNER@:rwxtTcy\n"   # has 'w' — forbidden
    "A::GROUP@:rxtTcy\n"
)
...
def test_diff_raw_passes_canonical_root():
    ...
    out = A.diff_raw(acls, host="lab-server")
...
def test_diff_raw_flags_unexpected_principal():
    text = (
        "# file: /srv/acl-view/lab_vm/raw\n"
        "A::OWNER@:rwa\n"
        "A:fdg:somebody_random@example.edu:rwx\n"
    )
...
REFINED_ROOT_OK = (
    "# file: /srv/acl-view/lab_vm/refined\n"
    "A::OWNER@:rwaDxtTnNcCy\n"
    "A::GROUP@:rwaDxtTnNcy\n"
    "A::EVERYONE@:tcy\n"
    "A:fdg:Administrators@example.edu:rwaDdxtTnNcCoy\n"
    "A::OWNER@:rwaDdxtTnNcCoy\n"
    "A:fdi:OWNER@:rwaDdxtTnNcCoy\n"
    "A:fdg:Users@example.edu:rxtncy\n"
    "A:dg:Users@example.edu:way\n"
)

REFINED_BC_DCIS_EXCEPTION = (
    "# file: /srv/acl-view/lab_vm/refined/bc_dcis\n"
    "A:fd:emucaki@example.edu:rwaDdxtTnNcCoy\n"
    "A:fd:OWNER@:rwaDdxtTnNcCoy\n"
    "A:fd:core_lead@example.edu:rwaDdxtTnNcCoy\n"
    "A:fdg:Administrators@example.edu:rwaDdxtTnNcCoy\n"
    "A::OWNER@:tcy\n"
