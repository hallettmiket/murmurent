...
def test_initialize_ssh_install_on_bare_repo_no_local_dir(isolated, tmp_path, monkeypatch):
    ...
    closes-the-loop case for #9: click `+ install` for lab-server on a
    GitHub repo you've never had locally and it ends up wigamig-ready
    on lab-server, in the Projects list, and in the Installations list.
    ...
    def fake_run(self, command, *, check=True, timeout=60):
        if "wigamig --version" in command:
            ...
            stdout = "\n".join([
                ...
                "repo:ok:cloned git@github.com:hallettmiket/freshrepo.git into /home/UWO/the_pi/repos/freshrepo",
                "cc_agent:ok:blacksmith -> wigamig/agents/blacksmith.md",
                "cc_claude_md:ok:created /home/UWO/the_pi/repos/freshrepo/CLAUDE.md",
            ]) + "\n"
        else:
            # The adopt_remote_clone script, identifiable by `DEST=` +
            # the CHARTER heredoc marker.
            stdout = "\n".join([
                "charter:ok:wrote /home/UWO/the_pi/repos/freshrepo/CHARTER.md",
                "cc_agent:ok:blacksmith -> wigamig/agents/blacksmith.md",
                "cc_claude_md:ok:already exists at /home/UWO/the_pi/repos/freshrepo/CLAUDE.md",
            ]) + "\n"
        return _remote_mod.RemoteResult(
            host="lab-server", command=command, returncode=0,
    ...
    assert "/home/UWO/the_pi/repos/freshrepo" in reg_text
