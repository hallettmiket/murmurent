...
from wigamig.core import onboarding as OB
...
def world(monkeypatch, tmp_path):
    ...
    CI.init_centre(name="Serenity", institution="U", founding_mayor="@tbrowne",
                   ...
                   data_server="biodatsci", write_sentinel=False)
    return tmp_path


def _make_lab(name="mh", pi="@mhallet", email="pi@x.edu"):
...
def test_onboards_when_pi_present(world):
    ...
    assert calls["channel"] == [("mh", "@mhallet", "pi@x.edu")]
...
def test_all_groups_when_no_arg(world):
    _make_lab(name="mh", pi="@mhallet", email="a@x.edu")
