...
def test_directory_idempotent(hub):
    H.upsert_directory(hub, "Western University", "Nirvana", "m@uwo.ca", "age1abc")
    assert H.upsert_directory(hub, "Western University", "Nirvana", "m@uwo.ca", "age1abc") == "unchanged"


def test_directory_update_on_key_change_matches_by_label(hub):
    H.upsert_directory(hub, "Western University", "Nirvana", "m@uwo.ca", "age1old")
    action = H.upsert_directory(hub, "Western University", "Nirvana", "m@uwo.ca", "age1new")
...
def test_readme_replaces_placeholder(hub):
    action = H.upsert_readme(hub, "Western University", "Nirvana", "m@uwo.ca", "age1abc")
    assert action == "updated"
    body = (hub / "README.md").read_text()
    assert "| Western University | Nirvana | m@uwo.ca | age1abc |" in body
    assert "_(added when live)_" not in body


def test_readme_idempotent_then_append(hub):
    H.upsert_readme(hub, "Western University", "Nirvana", "m@uwo.ca", "age1abc")
    assert H.upsert_readme(hub, "Western University", "Nirvana", "m@uwo.ca", "age1abc") == "unchanged"
...
def test_prepare_listing_full_flow(hub):
    ...
    res = H.prepare_listing(institution="Western University", name="Nirvana",
                            email="mhallet@uwo.ca", recipient="age1abc",
                            hub_dir=hub, runner=lambda *a, **k: None)
    assert res.cloned is False
    assert res.directory_action == "updated" and res.readme_action == "updated"
    assert res.row == "Western University (Nirvana)\tmhallet@uwo.ca\tage1abc"
...
def test_submit_pr_reuses_existing_pr(tmp_path):
    resp = {"api user": _Proc(0, "mhallet\n"),
            "pr create": _Proc(1, "", "a pull request already exists for mhallet:list-x"),
