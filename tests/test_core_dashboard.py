...
def test_login_resolve_non_leader_non_registrar_has_core_leader_false(world):
    ...
    gets is_core_leader=false. mhallet is the registrar in this fixture
...
def test_login_select_rejects_core_leader_for_non_leader(world):
    ...
    # below means @mhallet (the registrar here) IS a valid core_leader.
    res = client.post("/api/login/select", json={
...
def test_resolver_grants_registrar_implicit_core_leader_access(world):
    ...
    _seed_core()
    client = TestClient(create_app())
    res = client.get("/api/login/resolve?user=mhallet")  # mhallet is the registrar
    d = res.json()
...
def test_core_services_list_empty(world):
    ...
    _seed_core()
    client = TestClient(create_app())
    res = client.get("/api/core/biocore/services?user=mhallet")
    assert res.status_code == 200, res.text
    d = res.json()
    assert d["ok"] is True
...
def test_registrar_can_select_core_leader_role_and_routes_to_first_core(world):
    ...
    client = TestClient(create_app())
    res = client.post("/api/login/select", json={
        "handle": "mhallet", "role": "core_leader", "remember_user": False,
    })
    assert res.status_code == 200, res.text
    d = res.json()
