...
#
# Output mode: dir 0750 owned by root:ssmd-ud-vmlab, files 0640 — readable
...
if [[ ! -d "/root/data4/lab_vm" ]]; then
    echo "lab_sec_dump: /root/data4/lab_vm not mounted (the sudo-only NFSv4 mount)." >&2
    echo "  Ask the sysadmin to add it to /etc/fstab — the script needs real root" >&2
    echo "  permissions on the OneFS export, and the v3 /data mount squashes root." >&2
    exit 78
fi
...
# Member discovery: iterate /home/UWO/ (biodatsci LDAP convention).
...
# with group ssmd-ud-vmlab readable. We emit ONLY counts + a small
