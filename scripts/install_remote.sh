...
#
# Example: bash scripts/install_remote.sh lab-server
#          bash scripts/install_remote.sh lab-server --branch main
#
# Pre-reqs on the laptop:
#   - ~/.ssh/config has an alias for <ssh-host> with key auth working
#     (the script refuses to prompt for a password — if `ssh lab-server
